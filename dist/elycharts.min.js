 ...
 * Licensed under the MIT (http://creativecommons.org/licenses/MIT/) license.
 **********************************************************************/
(function(c){var a=c.elycharts.featuresmanager;var b=c.elycharts.common;c.elycharts.funnel={init:function(d){},draw:function(f){var j=f.paper;var e=f.opt;f.xmin=e.margins[3];f.xmax=e.width-e.margins[1];f.ymin=e.margins[0]+Math.abs(e.rh);for(var h in e.values){var d=e.values[h];var i=e.method=="width"?d[d.length-1]/(d[0]?d[0]:1):Math.sqrt(d[d.length-1]/(d[0]?d[0]:1)*Math.pow(1/2,2))*2;f.ymax=e.height-e.margins[2]-i*Math.abs(e.rh);var g=this.pieces(f,h,0,1,1,d)}a.beforeShow(f,g);b.show(f,g);a.afterShow(f,g);return g},pieces:function(o,n,d,g,f,t){var x,j=[],e=o.opt;var s=t[0]?t[0]:1;var m=d;var k=(g-d-e.topSector-e.bottomSector)/(t.length>1?t.length-1:1);var q=f;if((x=this.edge(o,m,q,true))){j.push({path:x,section:"Edge",attr:o.opt.edgeProps})}if(e.topSector>0&&(x=this.section(o,m,m=m+e.topSector,q,q))){j.push({path:x.path,center:x.center,rect:x.rect,section:"Sector",serie:"top",attr:o.opt.topSectorProps})}var u=[];for(var l=1;l<t.length;l++){var r=t[0]?t[l]:1;if((x=this.section(o,m,m=m+k,q,e.method=="width"?q=r/s*f:q=Math.sqrt(r/s*Math.pow(f/2,2))*2))){var p=b.areaProps(o,"Series",n,l-1)}u.push({path:x.path,center:x.center,rect:x.rect,attr:p.plotProps})}j.push({section:"Series",serie:n,paths:u,subSection:"Plot",mousearea:"paths"});if(e.bottomSector>0&&(x=this.section(o,m,m=m+e.bottomSector,q,q))){j.push({path:x.path,center:x.center,rect:x.rect,section:"Sector",serie:"bottom",attr:o.opt.bottomSectorProps})}if((x=this.edge(o,m,q,false))){j.push({path:x,section:"Edge",attr:o.opt.edgeProps})}return j},section:function(k,m,i,j,l){x1a=k.xmin+(k.xmax-k.xmin)*(j/-2+1/2);x2a=k.xmin+(k.xmax-k.xmin)*(j/2+1/2);x1b=k.xmin+(k.xmax-k.xmin)*(l/-2+1/2);x2b=k.xmin+(k.xmax-k.xmin)*(l/2+1/2);y1=k.ymin+(k.ymax-k.ymin)*m;y2=k.ymin+(k.ymax-k.ymin)*i;var f=(x2a-x1a)/2;var h=f/(k.xmax-k.xmin)*2*Math.abs(k.opt.rh);var e=(x2b-x1b)/2;var g=e/(k.xmax-k.xmin)*2*Math.abs(k.opt.rh);var d=[];d.push(["M",x1a,y1]);if(k.opt.rh!=0){d.push(["A",f,h,0,0,k.opt.rh>0?1:0,x2a,y1])}else{d.push(["L",x2a,y1])}d.push(["L",x2b,y2]);if(k.opt.rh!=0){d.push(["A",e,g,0,0,k.opt.rh>0?0:1,x1b,y2])}else{d.push(["L",x1b,y2])}d.push(["z"]);return{path:d,center:[(x2a+x1a)/2,(y2+y1)/2+(k.opt.rh>0?-1:+1)*(h+g)/2],rect:[x1a,y1,x2a,y2]}},edge:function(j,i,d,g){if((g&&j.opt.rh>=0)||(!g&&j.opt.rh<=0)){return false}x1=j.xmin+(j.xmax-j.xmin)*(d/-2+1/2);x2=j.xmin+(j.xmax-j.xmin)*(d/2+1/2);y=j.ymin+(j.ymax-j.ymin)*i;var k=(x2-x1)/2;var f=k/(j.xmax-j.xmin)*2*Math.abs(j.opt.rh);var e=[];e.push(["M",x1,y]);e.push(["A",k,f,0,0,j.opt.rh<0?1:0,x2,y]);e.push(["A",k,f,0,0,j.opt.rh<0?1:0,x1,y]);e.push(["z"]);return e}}})(jQuery);
/**********************************************************************
 * ELYCHARTS
 * A Javascript library to generate interactive charts with vectorial graphics.
 *
